...
def create(dto: PlayerCreateSchema) -> PlayerSchema:
    ...
    with db.create_session() as session:
        if player_repo.get_by_gto(session, dto.gto_id) is not None:
            raise EntityAlreadyExistsException("Player with this gto_id already exists")
...
def get_by_nickname(nickname: str) -> PlayerSchema:
    with db.create_session() as session:
        ...
        if player is None:
            raise EntityNotFoundException("Player with this nickname is not found")
        return PlayerSchema.model_validate(player)
...
def get_by_gto(gto_id: int) -> PlayerSchema:
    with db.create_session() as session:
        ...
        if player is None:
            raise EntityNotFoundException("Player with this gto id is not found")
        return PlayerSchema.model_validate(player)
